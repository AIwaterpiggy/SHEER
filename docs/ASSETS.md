...
Both dataset builders and one streamed example were checked locally; full
non-streaming preparation remains a server check. Source terms still apply.
...
the large checkpoint files were not downloaded or independently revalidated.
...
|---|---|---|
| SAMSum | `native_free_source6_population_matched_phase3c_candidate.pt` | Not supplied; public access path pending |
...
- **Multi-News:** [google/long-t5-tglobal-base, frozen revision](https://huggingface.co/google/long-t5-tglobal-base/tree/5093e24fd41835bd08e5817c58d55f060430c478).
  The local snapshot's full identity matched the archived N128 record.
- **SAMSum:** the recorded identifier is `t5-large`. A local-directory probe of
  ...
  **not** presented as a verified replacement. Check the original server
  tokenizer/cache and preparation recipe before finalizing its public instructions.
...
cached identifier without downloading anything. See [validation details](../VALIDATION.md).
